...
let rec model_check_ me ft  = 
	...
	let new_me = match ft.op with
		...
		| 'S' -> ( tree_until me2 (List.hd ch_strings) (List.nth ch_strings 1) )
	...
	new_me;;
(*	print_endline (string_from_ftree ft) ; 
*)
...
let test_me = {l=LabelC '+'; c=[{l=LabelS(letter "");c=[]};{l=LabelS(letter "pq");c=[]}]};;
let test_me = {l=LabelC '+'; c=[{l=LabelS(letter "p");c=[]};{l=LabelS(letter "p");c=[]}]};;
let test_ft = {op='-';ch=[{op='U'; ch=[{op='p';ch=[]};{op='-';ch=[{op='q';ch=[]}]}]}]};;
let test_ft = {op='-';ch=[{op='U'; ch=[{op='p';ch=[]};{op='-';ch=[{op='q';ch=[]}]}]}]};;

(*println_tree (test_me);;*)
model_check test_me test_ft;;
(*out_me=tree_until out_me "-p" "q";;
println_tree (out_me);;
let out_me = model_check_ test_me test_ft;;

print_endline (string_from_ftree test_ft);;
model
(
*)

(* let rec model_check_ me ft  = let me2 = (List.fold_left model_check_ me ft.ch) in 
	tree_until ttt "p" "q"
	match ft.op with
		  'U' -> tree_until ttt "p" "q" 
		| 'S' -> ( tree_until me2 (List.hd ch_strings) (List.nth ch_strings 1) )
	| '&' -> ( tree_and me2 (List.hd ch_strings) (List.nth ch_strings 1) )
		| '-' -> ( tree_not me2 (List.hd ch_strings) )
		| _ -> me2 
	let ch_strings = List.map string_from_ftree ft.ch in
*)	
(*let rec mirror t = {l = (match t.l with 
	LabelC c -> (match c with '>' -> LabelC '<' | '<' -> LabelC '>' | _ -> t.l)
	| LabelS s -> t.l) ; c=List.rev t.c}
	print_int (List.length ch_strings) ;
*)
...
print_string ( tree_to_string (tree_until ttt2 "p" "q"));;
*)
