...
module HashtblHue = Hashtbl.Make(Hue)
module MemoHue = struct
(* Boilerplate *)
  type t = int
  let to_id_h  = HashtblHue.create 9
  let from_id_a = ref [| Hue.empty |] (*or use array*)
  let last_id = ref (-1)
  let from_id i = (!from_id_a).(i)
  let to_id (x :Hue.t) : int = 
    if   HashtblHue.mem  to_id_h x 
    then HashtblHue.find to_id_h x
    else (
      last_id := (!last_id)+1;
      HashtblHue.add  to_id_h x (!last_id);
...
(* Wrappers *)
  let empty = to_id Hue.empty
  let is_empty = (memo (fun x -> (Hue.is_empty (from_id x))))
  let mem = (memo2 (fun x y -> (Hue.mem x (from_id y))))
  let add = (memo2 (fun x y -> (to_id (Hue.add x (from_id y)))))
  let singleton = (memo (fun x -> (to_id (Hue.singleton x))))
  let remove = (memo2 (fun x y -> (to_id (Hue.remove x (from_id y)))))
  let union = (memo2 (fun x y -> (to_id (Hue.union (from_id x) (from_id y)))))
  let inter = (memo2 (fun x y -> (to_id (Hue.inter (from_id x) (from_id y)))))
  let diff = (memo2 (fun x y -> (to_id (Hue.diff (from_id x) (from_id y)))))
  let compare = (memo2 (fun x y -> (Hue.compare (from_id x) (from_id y))))
  let equal = (memo2 (fun x y -> (Hue.equal (from_id x) (from_id y))))
  let subset = (memo2 (fun x y -> (Hue.subset (from_id x) (from_id y))))
  let iter = (fun x y -> (Hue.iter x (from_id y)))
  let for_all = (fun x y -> (Hue.for_all x (from_id y)))
  let exists = (fun x y -> (Hue.exists x (from_id y)))
  let filter = (fun x y -> (Hue.filter x (from_id y)))
  let cardinal = (memo (fun x -> (Hue.cardinal (from_id x))))
  let elements = (memo (fun x -> (Hue.elements (from_id x))))
  let min_elt = (memo (fun x -> (Hue.min_elt (from_id x))))
  let max_elt = (memo (fun x -> (Hue.max_elt (from_id x))))
  let choose = (memo (fun x -> (Hue.choose (from_id x))))
  let find = (memo2 (fun x y -> (Hue.find x (from_id y))))
  let to_string = (memo (fun x -> (Hue.to_string (from_id x))))
  let println = (fun x -> (Hue.println (from_id x)))
  let of_list = (memo (fun x -> (to_id (Hue.of_list x))))
  let closure_of = (memo (fun x -> (to_id (Hue.closure_of x))))
  let closure = Hue.closure
  let mpc = (memo2 (fun x y -> (Hue.mpc (from_id x) y)))
  let valid = (memo2 (fun x y -> (Hue.valid (from_id x) y)))
  let rx = (memo2 (fun x y -> (Hue.rx (from_id x) (from_id y))))
  let has_successor = (memo2 (fun x y -> (Hue.has_successor x (from_id y))))
  let filter_hues = (memo (fun x -> (Hue.filter_hues x)))
  let directly_fulfilled = (memo2 (fun x y -> (Hue.directly_fulfilled x y)))
  let fulfilled = (memo2 (fun x y -> (Hue.fulfilled x y)))
  let all_fulfilled = (memo (fun x -> (Hue.all_fulfilled x)))
  let all_hues = List.map to_id Hue.all_hues
  let hash = (memo (fun x -> (Hue.hash (from_id x))))
  let last_chars = (memo2 (fun x y -> (Hue.last_chars x y)))
  let ends_in = (memo2 (fun x y -> (Hue.ends_in x y)))
  let state_atom_local = (memo (fun x -> (Hue.state_atom_local x)))
  let state_atom = (memo (fun x -> (Hue.state_atom x)))
  let state_atoms = (memo (fun x -> (to_id (Hue.state_atoms (from_id x)))))
  let can_formula = (memo (fun x -> (Hue.can_formula x)))
  let can_formulas = (memo (fun x -> (to_id (Hue.can_formulas (from_id x)))))
...
let max_hues_in_colour = 
	...
	else int_of_float (log (float_of_int colour_limit) /. log (float_of_int (List.length MemoHue.all_hues)))
...
module Colour = struct
	...
	let () = assert (mem_f (ATOM 'p') (singleton(MemoHue.singleton(ATOM 'p'))))

	let to_string x = "{" ^ (String.concat ", " (List.map MemoHue.to_string (elements x))) ^ "}"

(* === Colour === *)
	let valid c =
		let arbitrary_hue = min_elt c in
		let can_f   = MemoHue.can_formulas arbitrary_hue in
		let state_a = MemoHue.state_atoms  arbitrary_hue in
		let sat_c1  = for_all (fun h-> 
(*i			let cf_h=MemoHue.can_formulas h in
			let yn = (cf_h=can_f) in
			printf "i%d  U%d %s \n" (MemoHue.cardinal  (MemoHue.inter cf_h can_f)) (MemoHue.cardinal  (MemoHue.union cf_h can_f)) (bool2str yn);
			printf "i%s  U%s %s \n" (MemoHue.to_string (MemoHue.inter cf_h can_f)) (MemoHue.to_string (MemoHue.union cf_h can_f)) (bool2str yn);
			printf "%s ?? %s %s \n" (MemoHue.to_string cf_h) (MemoHue.to_string can_f) (bool2str yn); i*)
			MemoHue.equal (MemoHue.can_formulas h) can_f &&
			MemoHue.equal (MemoHue.state_atoms  h) state_a 
		) c in
		let sat_c2 =
			MemoHue.for_all (fun f->
				match f with
...
module Instance = struct
  type t = Colour.t * MemoHue.t
  let compare = compare
  let to_string (c,h) = "Col: " ^ (Colour.to_string c) ^ "MemoHue: " ^ (MemoHue.to_string h)
end;;
	...
	List.filter (fun (c: Colour.t) ->
		not (
			Colour.exists (fun h ->
				List.for_all (fun d->
					(not (Colour.rx c d)) ||
					Colour.for_all (fun g-> not (MemoHue.rx h g)) d
				) colours
			) c 
		)
	) colours
	...
	List.iter (fun f -> match f with
		| UNTIL(a,beta) -> 
			...
			let new_colours = List.filter (fun c->
				Colour.for_all ( 
					fun h-> (MemoHue.mem f h) ==>  InstanceSet.mem (c,h) ful_b 
				) c
			) (!colours) in
			colours := new_colours 
		| _ -> ())
...
then "RESULT: is SATISFIABLE"
	...
	if (max_hues_in_colour < List.length MemoHue.all_hues) 
	then Printf.sprintf "Not satisfied, but large colours with more than %d (of %d) hues have been excluded\nRESULT: UNKNOWN" max_hues_in_colour (List.length MemoHue.all_hues) 
	else  
		"RESULT: UNsatisfiable"
let result = let num_hues = (List.length MemoHue.all_hues) in
